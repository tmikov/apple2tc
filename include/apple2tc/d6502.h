...
struct CPUOpcode {
  ...
  /// Base cycle cost: not counting the +1 a taken branch adds, and not
  /// counting the +1 an indexed read pays when it crosses a page. See
  /// cpuInstCycles() for the derivation.
  ///
  /// Defaulted (rather than left uninitialized) so that decodeOpcodeSlow()'s
  /// many two-field aggregate initializers -- which build a CPUOpcode purely
  /// to report (kind, addrMode) and never look at cycles -- stay legal and
  /// don't trip -Wmissing-field-initializers.
  uint8_t cycles = 0;

  static constexpr CPUOpcode invalid() {
    return {CPUInstKind::INVALID, CPUAddrMode::A, 0};
  }
};
...
/// Base cycle cost of an instruction, not counting the +1 a taken branch adds
/// or the +1 an indexed read pays when it crosses a page. Neither engine
/// models the page crossing, so this is the whole of the shared cycle model
/// apart from branches; see docs/plans/2026-08-12-accurate-cycles-design.md.
/// `kind` must not be CPUInstKind::INVALID -- there is no meaningful cost for
/// an instruction that was never really executed, and asking for one is a bug
/// at the call site, not a value to make up.
