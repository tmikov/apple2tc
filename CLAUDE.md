...
CMake-based build requiring C++17 and C11:
...
Tests are in `tests/` and run via `tests/run-tests.sh`. The script expects binaries at `../cmake-build-debug/` relative to the tests directory (CLion default build path). The workflow is: assemble `.s` files with `a6502`, decompile with `apple2tc -O3 --ir`, then diff against baseline `.ir` files.

```shell
cd tests
../cmake-build-debug/tools/a6502/a6502 trees.s trees.b33
../cmake-build-debug/tools/apple2tc/apple2tc trees.b33 -O3 --ir > trees-test.ir
