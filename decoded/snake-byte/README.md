...
| `snake-byte-cold.c` | The cold build's translation unit: stitches the above together and installs the entry state. |
| `snake-byte-cold-body.c` | Decompiler output, **hand-pruned and no longer regenerated**. Deletion only, apart from one changed initializer. Of its 3,046 lines, 968 are the last 89 unconverted blocks and the rest is data -- the game image, and the ROM image because `$E000`'s bytes are read as delay lengths. |
| `entry-state-inc.h` | The machine state at `$3750`: `$0000-$0802`, registers, video switch. Generated by `make-entry-state.sh`. |
